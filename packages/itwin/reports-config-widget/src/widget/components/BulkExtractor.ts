...
export default class BulkExtractor {
  ...
  private _reportMappings?: ReportMappingAndMapping[];
  ...
  public setReportMappings(reportMappings: ReportMappingAndMapping[]) {
    this._reportMappings = reportMappings;
  }
  ...
  private async fetchStates() {
    ...
    if (this._reportMappings && stateByIModelId.size > 0) {
  ...
  public async runIModelExtractions() {
    if (this._reportMappings) {
      const uniqueIModels = new Set(Array.from(this._reportMappings.map(x => x.imodelId)));
